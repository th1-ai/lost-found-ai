...
You are working inside **Lost & Found AI** ("The Finder") — Logs found items with photos, matches inbound guest claims to the log, handles the guest conversation, arranges paid return shipping with a courier, and closes the case with tracking..
...
means the agent drafts and queues, nothing more. This holds even after `approve`
or `edit` - shadow mode records that decision, it does not queue a real send or
ship; `python3 tools/review.py send` / `ship` still refuse and say so plainly.
Do not work around it. Do not suggest working around it. If a command is
blocked, that is the system doing its job — read the message, it says what
to do.
...
**Main workflow.** `workflows/10-match-claims.md` (`tools/run.py --once`). One
pass: read the found-items log and log anything new, turn each new guest
email into a structured claim (the one LLM call in this agent), score every
open claim against every logged item (pure Python, no model -
`tools/engine.py`), and expire any claim whose 14-day sweep has come due.

**Sub-agents.** None. `docs/sub-agents.md` says so plainly - this is one
self-contained loop, not a parent with children folded in.

**What always needs a human.**

- Every drafted return email, before it sends (`workflows/80-review.md`).
- Every shipment, as a **second**, separate click after the email has
  actually gone out (`tools/review.py ship`) - never combine these two steps
  for a guest, even if they seem eager to have it back tomorrow.
- **Any match touching jewellery or a document (passport, ID card, driving
  licence) - always, regardless of confidence.** `tools/review.py approve`
  and `edit` refuse a `[HIGH VALUE]` item outright without
  `--duty-manager-ack "<name>"`, and `ship` asks for the same ack a second
  time. This is the roster's "cant" promise and it is enforced in code -
  never suggest a way around it, and never fill in a name yourself. Get the
  actual duty manager to look at the match first.
- A same-family, different-category near-miss (reading glasses vs.
  sunglasses, the "confusable family" case) is never proposed as a match at
  all, however similar the wording - it stays open with a written reason
  instead. That is not a bug to work around.
